...
export class MerchPageComponent implements OnInit, OnDestroy {
  ...
  defineMetadata(): void {
    ...
    this.meta.addTag({ name: 'description', content: `${translationPipe.transform('METADATA_MERCH_DESCRIPTION')}` });
    this.meta.addTag({ name: 'keywords', content: `${translationPipe.transform('METADATA_MERCH_KEYWORDS')}` });
